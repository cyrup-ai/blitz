...
impl<Rend: WindowRenderer> View<Rend> {
    pub fn init(
        ...
        // Create window with visible set to false initially to avoid AccessKit panic
        let mut attributes = config.attributes;
        attributes.visible = false;
        let winit_window = Arc::from(event_loop.create_window(attributes).unwrap());

        // Make IME conditional on text input focus for better UX
        let has_focused_text_input = config.doc.has_focused_text_input();
        winit_window.set_ime_allowed(has_focused_text_input);
        ...
        #[cfg(feature = "accessibility")]
        let accessibility = AccessibilityState::new(event_loop, &winit_window, proxy.clone());

        // Now that accessibility is set up (or feature is disabled), make the window visible
...
impl<Rend: WindowRenderer> View<Rend> {
    pub fn resume(&mut self) {
        ...
        self.renderer.render(|scene| {
            println!("🚀 Inside renderer.render() callback - calling paint_scene()");
            paint_scene(scene, &self.doc, scale, width, height);
            println!("🚀 paint_scene() completed");
        });
    ...
    /// Update IME state based on current text input focus
    pub fn update_ime_state(&mut self) {
        let has_focused_text_input = self.doc.has_focused_text_input();

        if self.ime_enabled != has_focused_text_input {
            self.window.set_ime_allowed(has_focused_text_input);
            self.ime_enabled = has_focused_text_input;
        }
    }
