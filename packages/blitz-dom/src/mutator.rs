...
impl DocumentMutator<'_> {
    ...
    pub fn create_element(&mut self, name: QualName, attrs: Vec<Attribute>) -> usize {
        ...
        let node = match self.doc.get_node(id) {
            Some(node) => node,
            None => {
                eprintln!("Warning: Failed to get newly created element node {}", id);
                return id; // Return the ID even if we can't initialize style data
            }
        };
    ...
    pub fn set_node_text(&mut self, node_id: usize, value: &str) {
        let node = match self.doc.get_node_mut(node_id) {
            Some(node) => node,
            None => {
                eprintln!(
                    "Warning: Cannot set text for node {}: node not found",
                    node_id
                );
                return;
            }
    ...
    pub fn add_attrs_if_missing(&mut self, node_id: usize, attrs: Vec<Attribute>) {
        ...
        let element_data = match node.element_data_mut() {
            ...
            None => {
                eprintln!(
                    "Warning: add_attrs_if_missing called on non-element node {}",
                    node_id
                );
                return;
            }
    ...
    pub fn insert_nodes_before(&mut self, anchor_node_id: usize, new_node_ids: &[usize]) {
        let parent_id = match self.doc.nodes[anchor_node_id].parent {
            Some(id) => id,
            None => {
                eprintln!(
                    "Warning: Cannot insert nodes before anchor {}: anchor has no parent",
                    anchor_node_id
                );
                return;
            }
        ...
        self.add_children_to_parent(parent_id, new_node_ids, &|parent, child_ids| {
            let node_child_idx = match parent.index_of_child(anchor_node_id) {
                Some(idx) => idx,
                None => {
                    eprintln!(
                        "Warning: Anchor node {} not found in parent's children during insertion",
                        anchor_node_id
                    );
    ...
    pub fn insert_nodes_after(&mut self, anchor_node_id: usize, new_node_ids: &[usize]) {
        match self.next_sibling_id(anchor_node_id) {
            ...
            None => {
                let parent_id = match self.parent_id(anchor_node_id) {
                    Some(id) => id,
                    None => {
                        eprintln!(
                            "Warning: Cannot insert nodes after anchor {}: anchor has no parent",
                            anchor_node_id
                        );
                        return;
                    }
        ...
        if let Some(raw_src) = node.attr(local_name!("src")) {
            if let Ok(custom_paint_source_id) = raw_src.parse::<u64>() {
                ...
                match node.element_data_mut() {
                    ...
                    None => {
                        eprintln!(
                            "Warning: Cannot set canvas data for node {}: node is not an element",
                            target_id
                        );
                    }
                }
