...
pub struct Node {
    ...
    // Style generation tracking for smart invalidation
    /// Track when stylo styles change to invalidate taffy cache
    pub style_generation: std::sync::atomic::AtomicU32,
    /// Cached style generation to check if taffy style is still valid
    pub cached_style_generation: std::sync::atomic::AtomicU32,

    // Cursor caching fields
    cached_cursor: Cell<Option<CursorIcon>>,
    cursor_cache_valid: Cell<bool>,
}
...
impl Node {
    pub(crate) fn new(
        ...
        Self {
            ...
            style: UnsafeCell::new(Default::default()),
            has_snapshot: false,
            ...
            // Initialize style generation tracking
            style_generation: std::sync::atomic::AtomicU32::new(0),
            cached_style_generation: std::sync::atomic::AtomicU32::new(0),

            // Initialize cursor cache
            cached_cursor: Cell::new(None),
            cursor_cache_valid: Cell::new(false),
        }
    }
    ...
    /// Invalidate the cursor cache for this node
    pub fn invalidate_cursor_cache(&self) {
        self.cursor_cache_valid.set(false);
    }
    
    /// Get cached cursor if valid, otherwise return None
    pub fn get_cached_cursor(&self) -> Option<Option<CursorIcon>> {
        if self.cursor_cache_valid.get() {
            Some(self.cached_cursor.get())
        } else {
            None
        }
    }
    
    /// Set cached cursor and mark cache as valid
    pub fn set_cached_cursor(&self, cursor: Option<CursorIcon>) {
        self.cached_cursor.set(cursor);
        self.cursor_cache_valid.set(true);
    }
    ...
    /// Get immutable reference to style (thread-safe)
    pub fn style(&self) -> &Style {
        // SAFETY: This unsafe operation is justified by the following invariants:
        //
        // ## Safety Invariants:
        // 1. **Immutable Access Only**: This method only creates immutable references
        //    - Multiple concurrent readers are safe with UnsafeCell
        //    - No mutation occurs through this reference
        //    - Reference lifetime tied to &self, preventing use-after-free
        //
        // 2. **Valid Memory Layout**: The pointer from style.get() points to:
        ...
        //
        // 3. **No Concurrent Writers**: While this read occurs:
        //    - UnsafeCell coordinate access through external synchronization
        //    - Generation tracking ensures readers see consistent state
        //    - Writers use style_interior_mut() with proper coordination
        //
        // ## Preconditions:
        // - Node must remain alive for the returned reference lifetime
        // - Self reference must be valid and properly initialized
        // - No concurrent writers accessing style_interior_mut() for this node
        //
        // ## Postconditions:
        // - Returns immutable reference with same lifetime as &self
        // - Reference points to current style value (may be stale if cache invalid)
        // - Multiple concurrent calls to this method are safe
        //
        // ## Thread Safety:
        // - Safe for concurrent reads from multiple threads
        // - Coordination with writers through generation tracking system
        // - UnsafeCell interior mutability pattern with external synchronization
        unsafe { &*self.style.get() }
    }
    
    /// Get mutable reference to style (thread-safe single writer)
    pub(crate) fn style_mut(&mut self) -> &mut Style {
        self.style.get_mut()
    }
    
    /// Get mutable access to style through interior mutability (for cache updates)
    /// 
    /// # Safety
    /// 
    /// This method returns a raw mutable pointer to the style data stored in an `UnsafeCell`.
    /// The caller must ensure that no aliasing occurs when using this pointer.
    /// 
    /// ## Safety Invariants (MUST be maintained by caller)
    /// 
    /// 1. **No Concurrent Access**: At most one mutable reference can exist at any time.
    ///    Multiple readers OR one writer, never both simultaneously.
    /// 
    /// 2. **Lifetime Constraints**: The returned pointer is valid only while:
    ///    - The `Node` remains allocated and unchanged
    ///    - No other mutable access to the same style occurs
    ///    - The caller maintains exclusive access to the style data
    /// 
    /// 3. **Thread Safety**: This is NOT thread-safe. Callers must ensure:
    ///    - Only one thread accesses the style data at a time
    ///    - Proper synchronization (e.g., atomic generation counters) for coordination
    ///    - No data races between readers and writers
    /// 
    /// ## Safe Usage Patterns
    /// 
    /// ```rust,ignore
    /// // SAFE: Exclusive access with atomic coordination
    /// let style_ptr = node.style_interior_mut();
    /// unsafe {
    ///     *style_ptr = new_style; // OK - exclusive write
    /// }
    /// // Update generation counter AFTER style update for consistency
    /// node.cached_style_generation.store(new_gen, Ordering::Relaxed);
    /// ```
    /// 
    /// ## Undefined Behavior - DO NOT DO THIS
    /// 
    /// ```rust,ignore
    /// // UB: Concurrent access without synchronization
    /// let ptr1 = node.style_interior_mut();
    /// let ptr2 = node.style_interior_mut(); // UB if used simultaneously
    /// 
    /// // UB: Reading while writing
    /// let style_ref = node.style(); // Reading
    /// unsafe { *node.style_interior_mut() = new_style; } // UB - writing while reading
    /// 
    /// // UB: Accessing after node is moved/dropped
    /// let ptr = node.style_interior_mut();
    /// drop(node); // Node moved/dropped
    /// unsafe { *ptr = new_style; } // UB - use after free
    /// ```
    /// 
    /// ## Integration with Generation Tracking
    /// 
    /// When using this method for cache updates, follow this pattern:
    /// 1. Check generation numbers atomically
    /// 2. Obtain exclusive access via this method
    /// 3. Update style data atomically
    /// 4. Update cached generation AFTER style update
    /// 
    /// This ensures cache consistency and prevents race conditions.
    pub(crate) fn style_interior_mut(&self) -> *mut Style {
        self.style.get()
    }
}
...
impl Node {
    pub fn tree(&self) -> &Slab<Node> {
        ...
        //
        // ## Safety Invariants:
        // 1. **Valid Raw Pointer**: The self.tree field contains a valid pointer to a Slab<Node>
        //    - Pointer is set during Node initialization and never changed
        //    - Points to memory that remains valid for the Node's entire lifetime
        //    - Slab<Node> memory layout is stable and properly aligned
        //
        // 2. **Lifetime Consistency**: The returned reference has the same lifetime as &self
        //    - Slab<Node> outlives all Node references within it
        //    - Reference cannot outlive the node or the containing tree structure
        //    - Memory management ensures tree remains valid during node access
        //
        // 3. **Shared Ownership**: Multiple nodes can safely reference the same tree
        //    - All nodes in the tree share the same Slab<Node> reference
        //    - Tree structure provides coordinated access to all nodes
        //    - No mutation occurs through this immutable reference
        //
        // ## Preconditions:
        // - self.tree must point to a valid, initialized Slab<Node>
        // - Tree structure must remain alive for the returned reference lifetime
        // - Node must be part of a properly constructed tree structure
        //
        // ## Postconditions:
        // - Returns immutable reference to the tree containing this node
        // - Reference provides access to all nodes in the same tree
        // - Multiple concurrent calls to this method are safe
        //
        // ## Memory Safety:
        // - No use-after-free: Tree lifetime managed by external structure
        // - No double-free: Tree ownership managed externally
        // - No data races: Immutable access only through this method
        unsafe { &*self.tree }
    }
    ...
    pub fn with(&self, id: usize) -> &Node {
        match self.tree().get(id) {
            Some(node) => node,
            None => {
                eprintln!("Warning: Node with id {} not found in tree", id);
                // Return self as fallback to avoid panic
                self
            }
        }
    }
    ...
    pub fn node_debug_str(&self) -> String {
        ...
        let write_result = match &self.data {
        ...
        if let Err(_) = write_result {
            eprintln!(
                "Warning: Failed to write node debug string for node {}",
                self.id
            );
        }
        s
            ...
            NodeData::Element(data) => {
                ...
                for attr in data.attrs() {
                    ...
                    if let Some(color) = current_color.as_ref() {
                        if attr.value.contains("currentColor") {
                            writer.push_str(&attr.value.replace("currentColor", color));
                        } else {
    ...
    pub fn primary_styles(&self) -> Option<AtomicRef<'_, ComputedValues>> {
        ...
        // Use filter_map to safely extract the primary styles without unwrap()
        AtomicRef::filter_map(
            stylo_element_data,
            |data: &Option<StyloElementData>| -> Option<&ComputedValues> {
                data.as_ref()
                    .and_then(|d| d.styles.get_primary().map(|arc| arc.as_ref()))
            },
        )
    }
            ...
            .or_else(|| {
                if self.flags.is_inline_root() {
                    let element_data = match self.element_data() {
                        Some(data) => data,
                        None => {
                            eprintln!("Warning: Inline root node {} has no element data", self.id);
                            return None;
                        }
                    };
                    let layout = match element_data.inline_layout_data.as_ref() {
                        Some(layout_data) => &layout_data.layout,
                        None => {
                            eprintln!(
                    ...
                    // Implement cosmyc-text hit testing using Buffer.hit()
                    use blitz_text::Cursor as CosmicCursor;
                    // Access the cosmyc-text buffer from the layout data
                    let buffer = &layout;
                    // Perform hit testing on the cosmyc-text buffer
                    match buffer.hit_test(x, y) {
                        Some(CosmicCursor { line, index, .. }) => {
                            // Found a precise text position - return enhanced hit result
                            Some(HitResult {
                                node_id: self.id,
                                x: line as f32, // Store line in x for precise cursor position
                                y: index as f32, // Store char index in y for precise cursor position
                            })
                        },
                        None => {
                            // No precise hit found, but coordinates are within the element bounds
                            // Return basic hit result for the text element
                            Some(HitResult {
                                node_id: self.id,
                                x,
                                y,
