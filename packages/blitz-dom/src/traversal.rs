...
impl BaseDocument {
    ...
    pub fn next_node(&self, start: &Node, mut filter: impl FnMut(&Node) -> bool) -> Option<usize> {
        ...
        loop {
            ...
            // Next is next sibling or parent
                ...
                match parent.children.iter().position(|id| *id == node.id) {
                    Some(self_idx) => {
                    ...
                    None => {
                        eprintln!(
                            "Warning: Node {} not found in parent's children list during traversal",
                            node.id
                        );
                        // Continue with parent as fallback
                        look_in_children = false;
                        parent
                    }
                }
    ...
    pub fn previous_node(&self, start: &Node, mut filter: impl FnMut(&Node) -> bool) -> Option<usize> {
        let start_id = start.id;
        let mut node = start;
        let mut look_in_children = true;
        
        loop {
            // Previous is last child of previous sibling (or deepest descendant of that)
            let previous = if let Some(parent) = node.parent_node() {
                match parent.children.iter().position(|id| *id == node.id) {
                    Some(self_idx) if self_idx > 0 => {
                        // Previous is previous sibling's deepest descendant
                        let prev_sibling_id = parent.children[self_idx - 1];
                        let mut current = &self.nodes[prev_sibling_id];
                        
                        // Find the deepest rightmost descendant
                        while !current.children.is_empty() && look_in_children {
                            let last_child_id = current.children[current.children.len() - 1];
                            current = &self.nodes[last_child_id];
                        }
                        look_in_children = true;
                        current
                    }
                    Some(_) => {
                        // Previous is parent (no previous sibling)
                        look_in_children = false;
                        parent
                    }
                    None => {
                        eprintln!(
                            "Warning: Node {} not found in parent's children list during reverse traversal",
                            node.id
                        );
                        // Continue with parent as fallback
                        look_in_children = false;
            ...
            // Continue search from the last node in the tree
            else {
                look_in_children = true;
                let root = self.root_node();
                
                // Find the deepest rightmost descendant of the root
