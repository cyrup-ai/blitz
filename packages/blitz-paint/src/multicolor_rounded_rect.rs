...
impl BuildBezpath for BezPath {
    ...
    fn insert_arc(&mut self, arc: Arc) {
        ...
        if let Some(first_el) = elements.next() {
            match first_el {
                PathEl::MoveTo(a) if !self.elements().is_empty() => {
                    self.push(PathEl::LineTo(a));
                }
                el => self.push(el),
            }
            self.extend(elements);
        }
        // If elements is empty, it's a no-op (arc with no path elements)
    }
