...
use blitz_traits::{
    events::{BlitzImeEvent, DomEvent, DomEventData, EventState, UiEvent},
    net::NetProvider,
};
...
use crate::events::{
    BlitzKeyboardData, NativeClickData, NativeCompositionData, NativeConverter, NativeFormData, NativeFocusData,
};
...
impl EventHandler for DioxusEventHandler<'_> {
    fn handle_event(
        ...
        let event_data = match &event.data {
            ...
            DomEventData::Ime(ime_event) => match ime_event {
                BlitzImeEvent::Enabled => Some(wrap_event_data(NativeCompositionData {
                    data: String::new(),
                })),
                BlitzImeEvent::Preedit(text, _cursor) => {
                    Some(wrap_event_data(NativeCompositionData {
                        data: text.clone(),
                    }))
                }
                BlitzImeEvent::Commit(text) => Some(wrap_event_data(NativeCompositionData {
                    data: text.clone(),
                })),
                BlitzImeEvent::Disabled => Some(wrap_event_data(NativeCompositionData {
                    data: String::new(),
                })),
            },

            DomEventData::Focus => Some(wrap_event_data(NativeFocusData {})),
            
            DomEventData::Blur => Some(wrap_event_data(NativeFocusData {})),
            
            DomEventData::Change => Some(wrap_event_data(NativeFormData {
                value: String::new(),
                values: HashMap::new(),
            })),

            DomEventData::Submit => Some(wrap_event_data(NativeFormData {
                value: String::new(),
                values: HashMap::new(),
            })),
        };
